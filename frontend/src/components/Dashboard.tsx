...
import { api, type ControlState, type Equity, type Signal, type Status } from "../api";
...
export default function Dashboard() {
  ...
  const refresh = useCallback(async () => {
    const [st, cs, eq, sg] = await Promise.allSettled([
      api.status(),
      api.controlState(),
      api.equity(),
      api.signals(),
  ...
  // Live snapshot wins for fast-moving fields; REST fills the rest.
  ...
  return (
    <div className="min-h-full">
      <TopBar status={status} running={running} mode={mode} connected={connected} />
